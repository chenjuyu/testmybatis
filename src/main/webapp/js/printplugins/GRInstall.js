...
function Install_Detect()
{
    ...
    if (_ReportOK.Register == undefined) //if ((_ReportOK == null) || (_ReportOK.Register == undefined))
    {
        ...
        document.write('<strong><span style="color: #ff0000">此页面需要安装 锐浪报表插件 才能保证其正常运行</span></strong><br />');
            
        if( _gr_isIE )
            document.write('<strong> 如浏览器的顶部或底部出现提示条，左键点击提示条并运行加载项，按此方式安装最简便</strong><br />');
            
        document.write('<strong><a href="' + gr_InstallPath + '/grbsctl6.exe">下载报表插件安装包</a>，并安装。谷歌chrome、火狐Firefox、微软edge不支持插件，必须选用支持插件的浏览器</strong><br />');
        document.write('锐浪报表插件安装后，<a href="#" onclick="javascript:document.location.reload();">点击此处</a> 重新加载此页面。如果这些提示文字还出现，可能是当前浏览器不支持插件，<a href="http://www.rubylong.cn/gridreport/doc/plugins_browser.htm">点击</a>了解更多。');
        document.write('</div>');
        return false;
    }
